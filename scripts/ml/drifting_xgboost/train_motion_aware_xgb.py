#!/usr/bin/env python3
"""
PHASE 3G — EXPERIMENT B: Motion-aware XGBoost (prev_* features added)

Trains XGBoost to predict delta_lat and delta_lon from environmental features
PLUS the legitimately available prev_* motion features (prev_delta_lat,
prev_delta_lon, prev_speed, prev_bearing) computed strictly from real
predecessor observations (t-7 → t).

Samples WITHOUT a valid predecessor are EXCLUDED (spec §6).
...
Output (data/processed/ml/models/drifting_xgboost/):
  motion_aware_xgb_latitude.json
  motion_aware_xgb_longitude.json
  motion_aware_xgb_metadata.json
  motion_aware_xgb_predictions.csv

Output (outputs/ml/drifting_xgboost/):
  motion_aware_xgb_evaluation.json
...
# ── Feature design: env stack PLUS prev_* motion ─────────────────────────
FEATURE_COLS = [
    "lat", "lon",
...
def _filter_with_prev(df: pd.DataFrame) -> pd.DataFrame:
    """Keep only rows where prev_delta_lat is not NaN."""
    m = df["prev_delta_lat"].notna()
    kept = int(m.sum())
    dropped = int((~m).sum())
    log.info("  prev_delta_lat available: %d  excluded (no predecessor): %d", kept, dropped)
    return df[m].reset_index(drop=True), dropped


def main() -> int:
    print("=" * 70)
    print("PHASE 3G — EXPERIMENT B: Motion-aware XGBoost (env + prev_* features)")
    print("=" * 70)
    ...
    # Drifting filter + predecessor filter
    is_drift = lambda d: ~d["iceberg_id"].isin(list(GROUNDED_ICEBERGS))
    tr_d = train_df[is_drift(train_df)].reset_index(drop=True)
    va_d = val_df[is_drift(val_df)].reset_index(drop=True)
    te_d = test_df[is_drift(test_df)].reset_index(drop=True)

    print(f"  Raw drifting: train={len(tr_d)}  val={len(va_d)}  test={len(te_d)}")

    tr, dropped_tr = _filter_with_prev(tr_d)
    va, dropped_va = _filter_with_prev(va_d)
    te, dropped_te = _filter_with_prev(te_d)

    n_tr, n_va, n_te = len(tr), len(va), len(te)
    print(f"  After predecessor filter: train={n_tr}  val={n_va}  test={n_te}")
    print(f"  Test icebergs: {sorted(te.iceberg_id.unique())}")
    print(f"  C39 in test: {int((te.iceberg_id=='C39').sum())} (of which with_prev={int(te[(te.iceberg_id=='C39')].prev_delta_lat.notna().sum())})")
    print(f"  Using {len(FEATURE_COLS)} features (env + prev_delta_lat/lon, prev_speed, prev_bearing)")
    ...
    params_lat = _tune(X_tr, y_tr_lat, X_va, y_va_lat, "delta_lat")
    params_lon = _tune(X_tr, y_tr_lon, X_va, y_va_lon, "delta_lon")
    ...
    metrics = _evaluate("MotionAwareXGB", te, pred_lat, pred_lon)
    ...
    lat_m.save_model(MOD_DIR / "motion_aware_xgb_latitude.json")
    lon_m.save_model(MOD_DIR / "motion_aware_xgb_longitude.json")

    meta = {
        "experiment": "B",
        "model_name": "motion_aware_xgb",
        "description": "Motion-aware XGBoost (env + prev_* features, drifting-only, predecessor-filtered)",
        "feature_columns": FEATURE_COLS,
        "n_features": len(FEATURE_COLS),
        "target_columns": [TARGET_LAT, TARGET_LON],
        "drifting_train_n_raw": len(tr_d),
        "drifting_train_n": n_tr,
        "drifting_val_n_raw": len(va_d),
        "drifting_val_n": n_va,
        "drifting_test_n_raw": len(te_d),
        "drifting_test_n": n_te,
        "excluded_no_predecessor": {"train": dropped_tr, "val": dropped_va, "test": dropped_te},
    ...
    (MOD_DIR / "motion_aware_xgb_metadata.json").write_text(
    ...
    pred_df.to_csv(MOD_DIR / "motion_aware_xgb_predictions.csv", index=False)

    OUT_DIR.mkdir(parents=True, exist_ok=True)
    (OUT_DIR / "motion_aware_xgb_evaluation.json").write_text(
